    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 text-slate-900 dark:text-slate-50 overflow-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 p-8">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-4">
              {[
                ...
                { url: "https://twitter.com/mattmurrs", IconComponent: Twitter },
                { url: "https://www.linkedin.com/in/matthew-murray3", IconComponent: Linkedin }
              ].map((contact, index) => (
                <a 
                  key={index}
                  href={contact.url}
          ...
          >
            <div className={`w-6 h-0.5 bg-slate-800 dark:bg-white transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`}></div>
            <div className={`w-6 h-0.5 bg-slate-800 dark:bg-white transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`}></div>
            <div className={`w-6 h-0.5 bg-slate-800 dark:bg-white transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`}></div>
          </button>
        </div>
      </nav>
      ...
      <div className={`fixed inset-0 z-40 transition-all duration-300 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="absolute inset-0 bg-slate-900/20 dark:bg-slate-900/40 backdrop-blur-sm" onClick={() => setMenuOpen(false)}></div>
        
        <div className={`absolute top-0 right-0 w-80 h-full bg-white/90 dark:bg-slate-800/90 backdrop-blur-md shadow-2xl transform transition-transform duration-300 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="col-span-12 md:col-span-8 space-y-8">
              <div className="space-y-4">
                <h1 className="text-8xl md:text-9xl font-black tracking-tight">
                  <span className="block text-slate-800 dark:text-white">Matt Murray</span>
                    ...
                    <div className="relative">
                      <div className="w-16 h-16 bg-slate-900 dark:bg-slate-100 rounded-xl overflow-hidden">
                        ...
                        <svg viewBox="0 0 64 64" className="w-full h-full">
                          {/* Background */}
                          <rect width="64" height="64" fill="currentColor" className="text-slate-900 dark:text-slate-100" />
                          ...
                          <g stroke="currentColor" strokeWidth="0.3" className="text-white dark:text-slate-900">
                          ...
                          <g stroke="currentColor" strokeWidth="0.2" className="text-white dark:text-slate-900">
                          ...
                          <rect x="28" y="18" width="8" height="15" fill="currentColor" className="text-slate-900 dark:text-slate-100" stroke="currentColor" strokeWidth="0.5" />
                          
                          {/* Broadway diagonal */}
                          <line x1="20" y1="54" x2="44" y2="12" stroke="currentColor" strokeWidth="0.4" className="text-white dark:text-slate-900" />
                          
                          {/* Location pin */}
                          <circle cx="32" cy="35" r="2.5" fill="#ef4444" stroke="white" strokeWidth="1" />
                        </svg>
                      </div>
                    </div>
                    <div>
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm font-medium text-slate-800 dark:text-white">NYC</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {[
                {
                  ...
                  period: "2024 - Present",
                  description: "Growing the EigenCloud with early stage builders, specializing in EigenDA with L2 & Ethereum infrastructure",
                  highlights: ["Verifiable Compute", "Data Availability", "Ethereum Rollups"]
                },
                {
                  title: "Developer Relations Lead",
                  organization: "Intuition",
                  period: "2023 - 2024",
                  description: "Research and worked on incentivized shared knowledge graph leveraging decentralized identity primitives",
                  highlights: ["Knowledge Graphs", "Information Finance", "Decentralized Identity"]
                },
                {
                  title: "Site Reliability Engeineer",
                  organization: "Procter & Gamble",
                  period: "2021 - 2023",
                  description: "Developed monitoring and reliability systems across all of P&G's Direct to Consumer (DTC) stores monitoring the end to end store operations",
                  highlights: ["DTC", "Site Reliability", "E-commmerce Operations"]
                }
              ].map((role, index) => (
                  ...
                  <div className="border-l-4 border-slate-200 dark:border-slate-700 hover:border-purple-500 dark:hover:border-purple-400 transition-all duration-300 pl-8 py-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {role.highlights.map((highlight, highlightIndex) => (
                        <span key={highlightIndex} className="px-3 py-1 text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                          {highlight}
                        </span>
                      ))}
                    </div>
